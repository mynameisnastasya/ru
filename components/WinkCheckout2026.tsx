...
export default function WinkCheckout2026() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !data?.order) throw new Error(data?.error?.message || "Не удалось оформить заказ.");
      setOrder(data.order);
      setCart([]);
      try {
    ...
    <main className="wc26">
      ...
      <form className="wc26-layout" onSubmit={submit}>
        <div className="wc26-form">
          ...
          <section className="wc26-payment"><p>Оплата</p><h2>Заказ сначала сохраняем.</h2><span>До подтверждения оплаты заказ остаётся в статусе «Ждём оплату». Оплаченным он станет только после подтверждённого платежа.</span></section>
          {error && <div className="wc26-error">{error}</div>}
          <button className="wc26-submit" type="submit" disabled={submitting || !cart.length}>{submitting ? "Оформляем заказ…" : "Оформить заказ"}</button>
        </div>

        <aside className="wc26-summary"><p>Ваш WINK</p>{cart.length ? <><div className="wc26-lines">{cart.map(line => <article key={line.lineId}><div><b>{line.subtitle}</b><small>{configLine(line.config)}</small><div><button type="button" onClick={() => updateQty(line.lineId, -1)}>−</button><span>{line.qty}</span><button type="button" onClick={() => updateQty(line.lineId, 1)}>+</button></div></div><strong>{money(line.unitPriceMinor * line.qty)}</strong></article>)}</div><div className="wc26-total"><span>Товары</span><b>{money(total)}</b></div><small className="wc26-summaryNote">Доставка считается отдельно и не входит в сумму товаров.</small></> : <div className="wc26-empty"><h3>Пока пусто.</h3><p>Покажем то, с чем сложно ошибиться?</p><Link href="/shop">Смотреть хиты</Link></div>}</aside>
      </form>
    </main>
    <CheckoutStyles />
  </WinkPageFrame2026>;
