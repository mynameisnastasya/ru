...
let lang='en';try{lang=localStorage.getItem('guru-lang')||'en'}catch{}
if(!['en','ru'].includes(lang))lang='en';
const langButtons=[$('#langBtn'),$('#langBtnMobile')].filter(Boolean);
function applyLang(){document.documentElement.lang=lang;$$('[data-en][data-ru]').forEach(el=>{el.innerHTML=el.dataset[lang]});$$('[data-aria-en][data-aria-ru]').forEach(el=>el.setAttribute('aria-label',el.dataset[`aria${lang==='en'?'En':'Ru'}`]));langButtons.forEach(b=>{b.textContent=lang==='en'?'EN / RU':'RU / EN';b.setAttribute('aria-label',lang==='en'?'Switch language to Russian':'Переключить язык на английский')});if(menuBtn)menuBtn.setAttribute('aria-label',mobileMenu?.classList.contains('open')?(lang==='ru'?'Закрыть меню':'Close menu'):(lang==='ru'?'Открыть меню':'Open menu'));try{localStorage.setItem('guru-lang',lang)}catch{}update()}
